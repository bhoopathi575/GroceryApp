...
export const Card = (props) => {
    ...
    const handleDeletePress = async (id) => {
        ...
        const newItems = items.categories[catIndex].items.filter(
            (item) => item.id != id
        );
        ...
        const ItemView = (productItem) => {
            ...
            return (
                <View key={product.id} style={styles.Acard}>
                    ...
                    <Text style={{ color: colors.primary, fontWeight: "200" }}>
                    ...
                    >
                        <TouchableOpacity
                            style={styles.button}
                            onPress={() =>
                                props.navigation.navigate("AddModifyItems", {
                                    isEdit: true,
                                    category: item.name,
                                    product,
                                })
                        ...
                        >
                            <Text
                                adjustsFontSizeToFit={true}
                                style={{ color: "green", fontFamily: "bold", margin: 5 }}
                            >
                        ...
                        >
                            <Text
                                adjustsFontSizeToFit={true}
                                style={{ color: "red", fontFamily: "bold", margin: 5 }}
                            >
                ...
                >
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={async () => {
                                        const cartIndex = cart.findIndex(
                                            (cartItem) =>
                                                cartItem.username == auth.loginUserInfo.username
                                        );
                                        if (cartIndex == -1) {
                                            const newCart = [
                                                ...cart,
                                                new Cart(auth.loginUserInfo.username, product.price, [
                                                    new CartItem(
                                                        product.id,
                                                        product.name,
                                                        product.detail,
                                        ...
                                        const index = cart[cartIndex].items.findIndex(
                                            (cartItem) => cartItem.id == product.id
                                        );
                                        let newCart = [...cart];
                                        if (index != -1) {
                                            newCart[cartIndex].items[index] = new CartItem(
                                                product.id,
                                                product.name,
                                                product.detail,
                                                product.price,
                    ...
                    {filterItems.length == 0 && (
                        <View style={{ marginTop: 20, marginLeft: 30 }}>
                            <Text style={{ fontFamily: "italic", textAlign: "center" }}>
                                No Items Available!
                            </Text>
                        </View>
                    )}
